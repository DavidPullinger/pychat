...
def DECODE_RESPONSE(response):
    action = response[3:response.find(";")]
    hashtag = response[response.find(";")+1:response.find("//}")]
    jsonbody = response[response.find("//}")+3:]
...
def receive():
    ...
    #print(serverAddress)
    #DECODE_RESPONSE(modifiedMessage.decode())
    message = "{//CREATE_ACC;60c4fc5db9f52b6c3ec1f5d2f0d03878ba4ec4f8a506a5db77cc3e81195e7d8f//}ACCOUNT CREATED"
    print("message received")
    return DECODE_RESPONSE(message)
    #if hash not correct, send again-----
